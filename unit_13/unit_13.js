...
function f6() {
 let out = '';
 let in1 = document.querySelector('.i-61').value;
 let in2 = document.querySelector('.i-62').value;
...
function f12() {
    ...
    for (let key in a12) {
        if (input == a12[key]) delete a12[key];
    }
    f5(a12,'.out-12');
}
...
function f15() {
    ...
    for (let key in a15) {
        for (let i = 0; i < a14[key].length; i++) {
        out += a14[key][i] + ' ';
        }
    }
...
function f17() {
    ...
    for (let key in a17) {
        if (a17[key].age > 30) out += a17[key]['age'] + ' ';
    }
...
function f19() {
    ...
    for (let vetka in a19) {
        for (let i = 0; i < a19[vetka].length; i++) {
            if (a19[vetka][i].toLowerCase()  == input) {
                out = vetka;
                break;
            }
            //else out = ''; // фукнция перестает работать, если я снимаю коментарии с этой строки
        }
    }
...
function f20() {
    let out = '';
    for (let vetka in a20) {
        for (let i = 0; i < a20[vetka].length; i++) {
            if (a20[vetka][i][1] == 2) {
            out += a20[vetka][i][0] + ' ';
            }
        }
